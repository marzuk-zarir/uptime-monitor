...
// List all file form and return a files array
db.listFiles = async (subDir, callback) => {
    try {
        const files = await fs.readdir(`${db.basedir}/${subDir}/`)
        if (files?.length > 0) {
            const filesArray = []
            for (const file of files) {
                filesArray.push(file.replace('.json', ''))
            }
            callback(null, filesArray)
        } else {
            throw Error('There was no file in this directory')
        }
    } catch (error) {
        callback(error.message, null)
    }

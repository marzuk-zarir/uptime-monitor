            ...
            checksArray.forEach((check) => {
                db.readData('check', check, (readErr, readData) => {
                    if (!readErr && readData) {
                        ...
                        callback('Error: when read a single file form check directory')
                    }
                })
            })
        } else {
    ...
    db.updateData('check', checkData.checkId, checkData, (upErr, upStatus) => {
        if (!upErr && upStatus) {
            ...
            if (isAlertWant) {
                ...
                console.log('Alert is not needed.Bcz there is no state change')
            }
        } else {
            console.log("Error: Couldn't update lastCheck and state for single check file")
        }
    })
}
...
worker.sendAlertToUser = (checkData) => {
    ...
    sendSms(checkData.phone, message, (err) => {
        if (!err) {
            console.log(`User is alert with via SMS - ${message}`)
        } else {
            console.log("Error: Couldn't send SMS to the user")
        }
    })
}
